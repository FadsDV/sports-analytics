        ...
        <Card title="Value Intelligence">
          <div className="grid grid-cols-2 gap-1.5 mb-2">
            {([
              ...
              <div key={item.label} className="bg-[#0d1827] rounded-lg px-2.5 py-2 flex flex-col gap-1.5">
                <span className="text-[10px] font-medium text-[#4B5563] uppercase tracking-wide">{item.label}</span>
                <span className="text-lg font-black text-[#1F2937] tabular-nums">{item.line}</span>
                <Badge color="gray">locked</Badge>
              </div>
            ))}
          </div>
          <p className="text-[10px] text-[#374151] text-center">Odds integration coming soon</p>
        </Card>
              ...
              return (
                <div key={key} className="mb-2.5 last:mb-0">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-[#D1D5DB] font-semibold tabular-nums w-8">{hv}</span>
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            {KEY_STATS.filter(k => row.stats[k] != null).slice(0, 3).map(k => (
                              <span key={k} className="text-[10px] tabular-nums">
                                ...
                                <span className={k === "D" && elite ? "text-[#3B82F6] font-bold" : "text-[#9CA3AF]"}>
                                  {row.stats[k]}
                                </span>
                              </span>
                            ))}
