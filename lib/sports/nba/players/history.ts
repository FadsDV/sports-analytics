 ...
 *   Stale (30 min–4 h) → incremental: only fetch summaries for new games
 *   Expired (> 4 h)    → full refetch across all seasons
 *
 * Coverage: 3 ESPN seasons (preseason, regular, play-in, playoffs).
 * No per-season game cap — file cache keeps subsequent loads instant.
 */
...
function mapSeasonType(ev: any): NBASeasonType | null {
  // ESPN NBA: seasonType is a top-level object { type: 2, name: "Regular Season" }
  // season is { year: 2025, displayName: "2024-25" } (no type field)
  const type = Number(ev.seasonType?.type ?? ev.season?.type ?? 0);
  const name  = String(ev.seasonType?.name ?? ev.season?.name ?? "").toLowerCase();
...
function extractCompletedGames(
  ...
  teamId:     string,
  espnSeason: number,
  afterDate?: string    // incremental: only games strictly after this date
): ScheduleGame[] {
  ...
  for (const ev of events) {
    ...
    // ESPN returns score as { value: 109.0 } object or plain number
    const ourScore = Number(ours.score?.value ?? ours.score ?? NaN);
    const oppScore = Number(opp.score?.value  ?? opp.score  ?? NaN);
    const result: "W" | "L" | null = !isNaN(ourScore) && !isNaN(oppScore)
      ? ourScore > oppScore ? "W" : "L"
      : null;
...
function parseShotSplit(raw: string | null | undefined): {
  ...
  return {
    m:   !isNaN(m) ? m : null,
    a:   !isNaN(a) ? a : null,
    pct: !isNaN(m) && !isNaN(a) && a > 0 ? Math.round((m / a) * 1000) / 10 : null,
  };
}

function extractPlayerStats(summary: any, athleteId: string): Record<string, string | null> | null {
...
export async function fetchNBAPlayerHistory(
  ...
  if (cached && cached.entry.teamId === teamId) {
    ...
    if (!isStale) {
      ...
      if (lastDate) {
        ...
        const newScheduleGames = extractCompletedGames(currentEvents, teamId, currentYear, lastDate);

        if (newScheduleGames.length === 0) {
          // No new team games — refresh timestamp and return cached
          await writePlayerCache(athleteId, teamId, entry.games, entry.seasonsIncluded);
          return { games: entry.games, seasonsIncluded: entry.seasonsIncluded };
        }

        console.info(`[SportsPulse] NBA incremental — ${newScheduleGames.length} new game(s) to check`);
  ...
  console.info(
    `[SportsPulse] NBA boxscore history FULL — team:${teamId} athlete:${athleteId} ` +
    `seasons:${espnSeasons.join(",")}`
  );
  ...
  if (allScheduleGames.length === 0) {
    console.info(`[SportsPulse] NBA — no completed games found for team:${teamId}`);
    return { games: [], seasonsIncluded: [] };
  }

  console.info(`[SportsPulse] NBA — fetching ${allScheduleGames.length} game summaries (batch ${BATCH_SIZE})`);
  const summaries   = await fetchSummariesBatched(allScheduleGames);
  const playerGames = buildPlayerGames(allScheduleGames, summaries, athleteId);
  const seasonsIncluded = seasonsFromGames(playerGames);

  await writePlayerCache(athleteId, teamId, playerGames, seasonsIncluded);

  console.info(
    `[SportsPulse] NBA FULL DONE — athlete:${athleteId} ` +
    `games:${playerGames.length} seasons:[${seasonsIncluded.join(",")}]`
  );
