 ...
 * Stores fully-normalized NBAPlayerGame[] so repeated drawer opens are instant.
 *
 * Strategy:
...
interface CacheEntry {
  ...
  teamId:          string;
  lastGameDate:    string;   // date of most recent game (YYYY-MM-DD), "" if no games
  games:           NBAPlayerGame[];
  seasonsIncluded: number[];
}

export const CACHE_VERSION  = 3;                          // bump when NBAPlayerGame shape changes
export const FRESH_MS       = 30 * 60 * 1000;            // 30 min — no check
