    ...
    <div className="max-w-3xl">
      ...
      <div className="bg-[#0f172a] border border-[#1e293b] rounded-2xl p-6">
        ...
        <p className="text-sm text-gray-400">
          We could not load player data for <span className="text-white">{athleteId}</span> in{" "}
...
export default async function PlayerPage({
  ...
  const seasonStatEntries = profile.seasonStats
    ? Object.entries(profile.seasonStats)
    : [];
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {seasonStatEntries.map(([key, value]) => (
              ...
              >
                ...
                <div className="text-[11px] text-gray-500 mt-1 uppercase tracking-widest">
                  {key}
                </div>
              </div>
            ))}
...
function GameLogTable({ gamelog }: { gamelog: NormalizedGameLog[] }) {
  const statKeys = Array.from(
    new Set(gamelog.flatMap((g) => Object.keys(g.stats)))
  );
      ...
      <table className="w-full text-sm min-w-[480px]">
        <thead>
          <tr className="border-b border-[#1e293b]">
            <th className="text-left py-2 px-2 text-xs font-medium text-gray-500 whitespace-nowrap">
              Date
            </th>
